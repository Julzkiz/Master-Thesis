...
namespace Gecko
{
    internal class Model_Calculation_Stiffness_Matrix_2D_Bar
    {
        ...
        foreach (Load load in model.loads) //maybe just use V.Dense(DOFs.Count*d) -> vector of x length
        {
            ...
            L_vector[nodeid * d + 1] = load.vector.Z; //kN
        }
        ...
        foreach (Element bar in model.bars) //go through every bar and assign them a stiffness, start and end, and length.
        {
            ...
            double cos = (bar.axis.To.X - bar.axis.From.X) / (bar.length); //m-m/m
            double sin = (bar.axis.To.Z - bar.axis.From.Z) / (bar.length);
